...
export async function youtubeHandler(req, res) {
  ...
  try {
    ...
    writeStream.on('finish', () => {
      // בדיקת קיום הקובץ לפני שליחת התגובה
      fs.access(videoPath, fs.constants.F_OK, (err) => {
        ...
        // הגדרת טיימר למחיקת הקובץ לאחר 5 דקות
        setTimeout(() => {
        ...
        res.json({
          videoId, // הוספת videoId לתשובה
          title,
          uploadDate,
          viewCount,
          videoPath: `/videos/${videoId}.mp4`,
          message: `הסרטון הוכן בהצלחה (${(videoSize / 1048576).toFixed(1)}MB). הקובץ יימחק לאחר 5 דקות.`
        });
      });
    });
